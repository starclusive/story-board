...
<img loading="auto" src="${get(itemData, 'preview')}" />
                                ...
                                viewerItem(storyData, currentStoryItem) {
                                    console.log(storyData.items[currentStoryItem].viewersDetails, 'current story item')
...
<a class="close" tabIndex="2">
<img src="assets/img/Layer 3.svg" style="height:30px;width:30px">
</a>
</div>
${storyData.ownStory ?
`<div>
<h4>Views:${storyData.items[currentStoryItem].totalViewCount}</h4>
<h2>${storyData.items[currentStoryItem].viewersDetails.length > 0 ? 
each(storyData.items[currentStoryItem].viewersDetails,(i,item) => {
`${item.name}`
})
:''}</h2>
</div>` : ''}
